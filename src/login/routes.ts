...
const postCredentials = (formData: FormData, session: string) =>
  postForm(LoginPageUrl, formData, withSession(session)).mapError(
    makeNetworkError
  );
...
router.get('/', async (req, res) => {
  ...
  const redirectUrl = Operation.combineTwo(formData, session)
    .bindAsync(([formData, session]) => postCredentials(formData, session))
    .bind((res) => tryGetRedirectUrl(res).toEither(noRedirectError));

  const loginResponse = Operation.combineTwo(session, redirectUrl).bindAsync(
    ([session, url]) =>
      get(url, withSession(session)).mapError(makeNetworkError)
  );
